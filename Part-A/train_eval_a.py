...
def load_data(train_dir, test_dir, augment=False, train_batch_size=16, num_workers=8):
    ...
    if augment:
        ...
        train_loader = DataLoader(combined_train, batch_size=train_batch_size, shuffle=True,
                                  num_workers=num_workers, pin_memory=True, persistent_workers=True)
    else:
        train_loader = DataLoader(train_subset, batch_size=train_batch_size, shuffle=True,
                                  num_workers=num_workers, pin_memory=True, persistent_workers=True)

    val_loader = DataLoader(val_subset, batch_size=train_batch_size, shuffle=True,
                            num_workers=num_workers, pin_memory=True, persistent_workers=True)
...
class SimpleCNN(nn.Module):
    def __init__(self, input_channels, num_classes, num_filters, filter_sizes,
        ...
        for i in range(5):
            layers.append(nn.Conv2d(in_ch, out_ch, kernel_size=filter_sizes[i], stride=1, padding=1))
            if use_batchnorm:
...
def aggregate_sweep_results(sweep_id, project, entity):
    # Initialize a wandb run for artifact logging
    run = wandb.init(project=project, entity=entity, job_type="aggregation", reinit=True)
    api = wandb.Api()
    ...
    for r in runs:
        ...
        cfg = r.config
        final_acc = sum_json.get("Validation Accuracy", sum_json.get("Train Accuracy"))
        records.append({
            "run_id": r.id,
    ...
    ax1.plot(df["Experiment"], df["Validation Accuracy"], marker='o')
    ax1.set(xlabel="Experiment", ylabel="Validation Accuracy", title="Accuracy vs Experiment")
    plt.tight_layout()

    # Plot 2: Parallel Coordinates
    df_p = df.copy()
    df_p["Validation Accuracy"] = df_p["Validation Accuracy"].round(2).astype(str)
    from pandas.plotting import parallel_coordinates
    fig2, ax2 = plt.subplots(figsize=(10,6))
    parallel_coordinates(
        df_p, class_column="Validation Accuracy",
        cols=["num_filters","dense_neurons","dropout","filter_multiplier","learning_rate","epochs","Validation Accuracy"],
        ax=ax2
    )
    ...
    num_cols = ["num_filters","dense_neurons","dropout","filter_multiplier","learning_rate","epochs","Validation Accuracy"]
...
def main(args):
    if wandb.run is None:
        wandb.init(project="Alik_Final_DA6401_DeepLearning_Assignment2", entity="cs24m007-iit-madras", config=vars(args))
    train_loader, val_loader, test_loader = load_data(
        args.train_dir, args.test_dir,
        augment=(args.data_augmentation=="Yes")
    )
    print(f"Train len: {len(train_loader.dataset)}, Val len: {len(val_loader.dataset)}, Test len: {len(test_loader.dataset)}")
    model = SimpleCNN(
        input_channels=3, num_classes=10,
        num_filters=args.num_filters,
        filter_sizes=[args.filter_size_1,args.filter_size_2,args.filter_size_3,args.filter_size_4,args.filter_size_5],
...
# ---------------------------
def sweep_run():
    run = wandb.init(project="Alik_Final_DA6401_DeepLearning_Assignment2", entity="cs24m007-iit-madras")
    config = run.config
...
sweep_config = {
    ...
    'parameters':{
        ...
        'filter_size_3':{'value':3}, 'filter_size_4':{'value':3}, 'filter_size_5':{'value':3},
...
# ---------------------------
def parse_arguments():
    parser = argparse.ArgumentParser(description="Train CNN with wandb sweep and aggregation")
    ...
    parser.add_argument('--activation', type=str, default='ReLU', choices=['ReLU','GELU','SiLU','Mish'])
    parser.add_argument('--dense_neurons', type=int, default=128)
    parser.add_argument('--dropout', type=float, default=0.0)
    parser.add_argument('--batch_norm', type=str, default='No', choices=['Yes','No'])
...
if __name__ == '__main__':
    ...
    if args.aggregate:
        ...
        if args.sweep:
            ...
            sweep_id = wandb.sweep(sweep_config, project="Alik_Final_DA6401_DeepLearning_Assignment2")
